...
export default async function Storefrontcontact({ params }: Props) {
  ...
  try {
    // Find the contact page (slug: 'contact' or 'products')
    const contactPage = await prisma.pageDesign.findFirst({
      where: {
        tenant: {
          subdomain
        },
    ...
    if (contactPage) {
      const targetSlug = contactPage.slug?.trim()
      const normalizedSlug = targetSlug?.toLowerCase()

      if (targetSlug && normalizedSlug && !['contact', ''].includes(normalizedSlug)) {
        redirect(`/storefront/${subdomain}/${targetSlug}`)
      }

      // When the only published page is also the query slug (e.g. /contact),
      // render it directly instead of redirecting to avoid infinite loops.
      const components = (contactPage.publishedContent ?? contactPage.content) as unknown as ComponentData[]
      ...
      const storefront = {
        id: contactPage.tenant.id,
        subdomain: contactPage.tenant.subdomain,
        name: contactPage.tenant.name,
        settings: contactPage.tenant.settings as Record<string, unknown> | undefined,
        primaryColor: contactPage.tenant.primaryColor ?? undefined,
        secondaryColor: contactPage.tenant.secondaryColor ?? undefined
      }
    ...
    // If no contact page found, show products for this tenant or a friendly message
    const products = await prisma.product.findMany({ where: { tenantId: tenant.id, isActive: true }, take: 20 })
    const hasHeader = false
        if (products && products.length > 0) {
      // Use the same ProductGrid for a nicer default contact experience
...
<>
        ...
        <main className="w-full">
          <div className="w-full max-w-7xl mx-auto px-4 md:px-8 lg:px-12 py-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-6">{tenant.name} contact</h1>
...
<FooterSection storefront={{
    ...
    console.error('Error loading storefront contact:', error)
    return (
