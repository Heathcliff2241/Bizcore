...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  const handleLogout = async () => {
    ...
    if (storefront?.subdomain) {
      ...
      signOut({ callbackUrl: `/storefront/${storefront.subdomain}` });
    } else {
      try {
        await fetch('/api/auth/clear-session', { method: 'POST', credentials: 'include' })
      } catch (err) {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-20 right-4 md:right-8 w-72 bg-white rounded-2xl shadow-2xl z-[9999] overflow-hidden"
          >
            {/* Close button */}
            <button
              onClick={onClose}
